...
import { Wallet, Link as LinkIcon, Gift, Zap, Users, CheckCircle2, Copy, Unlink, X, AlertTriangle, Send, History } from 'lucide-react';
...
const ProfileSection = ({ user, refreshUser Data }) => {
  ...
  const handleClaimRewards = async () => {
    if (isBanned) {
      toast({
        ...
        description: 'You cannot request a withdrawal because your account is banned.',
        variant: 'destructive',
        className: 'bg-[#1a1a1a] text-white',
      });
      return;
    ...
    // Example: await requestWithdrawal(user.id, withdrawAmount);
    toast({
  ...
  return (
    <div className="relative w-full h-[100dvh] bg-[#0f0f0f] text-white">
      ...
      <div className="flex justify-between items-center p-4 bg-[#1a1a1a] rounded-lg mb-4">
        <span className="text-lg font-bold">Exchange Rate:</span>
        <span className="text-lg">100,000 STON = 0.5 TON</span>
      </div>

      {/* History Icon */}
      <div className="absolute top-4 right-4">
        <Button variant="ghost" onClick={() => {/* Logic to open transaction history */}}>
      ...
      <div className="flex flex-col items-center justify-center px-4 overflow-y-auto" style={{ height: '100dvh', paddingTop: isBanned ? '112px' : '0' }}>
        <div className="w-full max-w-md flex flex-col items-center gap-6">
          ...
          <div className="w-full mt-6 text-center">
            ...
            {user.wallet ? (
              <div className="flex items-center bg-white/5 px-4 py-3 rounded-xl text-sm w-full justify-between gap-2">
                ...
                <span className="truncate font-mono px-2 text-white select-text text-base text-left w-full" title={user.wallet} style={{ userSelect: 'text' }}>
                  {user.wallet.substring(0, 10)}...{user.wallet.substring(user.wallet.length - 4)}
                </span>
                <button type="button" className="flex items-center p-1.5 rounded-full transition hover:bg-sky-400/20 active:scale-95" aria-label="Copy Wallet Address" title={copying ? "Copied!" : "Copy Wallet Address"} onClick={handleCopyWallet}>
                  <Copy className={`h-5 w-5 ${copying ? 'text-green-400' : 'text-gray-400'} transition`} />
                </button>
                <button type="button" className="flex items-center p-1.5 rounded-full transition hover:bg-red-400/20 active:scale-95 ml-1" aria-label="Disconnect Wallet" title="Disconnect Wallet" onClick={handleDisconnectWallet}>
          ...
          <Button variant="ghost" className="mt-4 w-full" onClick={() => setShowClaimDialog(true)}>
        ...
        {showDialog && (
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1c1c1c] text-white w-[90%] max-w-sm p-6 rounded-xl shadow-xl relative">
              <button className="absolute top-3 right-3 text-gray-400 hover:text-white" onClick={() => setShowDialog(false)}>
                <X className="w-5 h-5" />
              </button>
              <h2 className="text-lg font-semibold mb-4">Enter your TON Wallet</h2>
              <Input value={walletInput} onChange={(e) => setWalletInput(e.target.value)} placeholder="EQ... or UQ..." className="mb-4 text-white placeholder:text-gray-400 bg-[#0f0f0f] border border-gray-700" />
        ...
        {showClaimDialog && (
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1c1c1c] text-white w-[90%] max-w-sm p-6 rounded-xl shadow-xl relative">
              <button className="absolute top-3 right-3 text-gray-400 hover:text-white" onClick={() => setShowClaimDialog(false)}>
                <X className="w-5 h-5" />
              </button>
              <h2 className="text-lg font-semibold mb-4">Request Withdrawal</h2>
              <p className="text-sm mb-2">Connected Wallet: {user.wallet}</p>
              <Input value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} placeholder="Amount to withdraw" className="mb-4 text-white placeholder:text-gray-400 bg-[#0f0f0f] border border-gray-700" />
              <p className="text-sm mb-2">Estimated TON: {(withdrawAmount / 100000 * 0.5).toFixed(4)} TON</p>
